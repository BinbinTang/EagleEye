...
public class AndroidBootImageUnpacker extends DiskImageUnpacker
{
	...
	protected Task<ArrayList<FileEntity>> createTask()
	{
		return new Task<ArrayList<FileEntity>>()
		{
			...
			protected ArrayList<FileEntity> call() throws Exception
			{
				...
				File file = AndroidBootImageUnpacker.this.formatDescription.getFile();
				AndroidBootImageUnpacker.this.fileInputStream = new FileInputStream(file);
				AndroidBootImageUnpacker.this.inputStream = new DataInputStream(AndroidBootImageUnpacker.this.fileInputStream);
				
				byte[] inputBytes = new byte[8];
				AndroidBootImageUnpacker.this.inputStream.readFully(inputBytes);
				...
				AndroidBootImageUnpacker.this.inputStream.readFully(unsignedHeaderBytes);
				...
				AndroidBootImageUnpacker.this.inputStream.readFully(productNameBytes);
				...
				AndroidBootImageUnpacker.this.inputStream.readFully(commandLineBytes);
				...
				AndroidBootImageUnpacker.this.inputStream.readFully(unsignedIdBytes);
				...
				if (!hasMTDDefinition)
				{
					...
					AndroidBootImageUnpacker.this.inputStream.close();
					return null;
				}
